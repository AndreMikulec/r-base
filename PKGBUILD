...
build() {
  ...
  # always keep sending G_FLAG="-ggdb -Og -g3".
  # The make runner, everytime, should not be bothered to set
  # the make flag G_FLAG to G_FLAG="-ggdb -Og -g3"
  #
  ...
  then
    sed -i -e "s/-gdwarf-2/-ggdb -Og -g3/" ${srcdir}/build32/src/gnuwin32/fixed/etc/Makeconf
    # WOULD HAVE ACCIDENTALLY done WHEN not DEBUG=T set DEBUGFLAG = -ggdb -Og -g3 # so that is wrong!
    # sed -i -e "s/^\s*DEBUGFLAG\s*?\?+\?=.*/DEBUGFLAG = -ggdb -Og -g3/" ${srcdir}/build32/src/gnuwin32/fixed/etc/Makeconf
  else
    echo -e "\n" >> ${srcdir}/build32/src/gnuwin32/fixed/etc/Makeconf
    echo "DEBUGFLAG = -ggdb -Og -g3" >> ${srcdir}/build32/src/gnuwin32/fixed/etc/Makeconf
  fi
  echo -e "\n"                                 >> ${srcdir}/build32/src/gnuwin32/fixed/etc/Makeconf
  ...
  then
    ...
    # WOULD HAVE ACCIDENTALLY done WHEN not DEBUG=T set DEBUGFLAG = -ggdb -Og -g3 # so that is wrong!
    # sed -i -e "s/^\s*DEBUGFLAG\s*?\?+\?=.*/DEBUGFLAG = -ggdb -Og -g3/" ${srcdir}/R-source/src/gnuwin32/fixed/etc/Makeconf
  else
